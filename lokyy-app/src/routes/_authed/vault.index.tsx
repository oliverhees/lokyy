...
import { VaultStatusHeader } from '@/components/vault-status-header'
      ...
      <div className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Dein persönliches Markdown-Vault. Lokal-only oder per Git/Forgejo synchronisiert.
          </p>
        </div>
        <Card>
          <CardContent className="space-y-4 p-6 text-center" data-testid="vault-empty">
            <BrainIcon className="mx-auto size-12 text-muted-foreground/50" />
            <p className="text-sm font-medium">Noch kein Vault eingerichtet</p>
            <p className="mx-auto max-w-md text-xs text-muted-foreground">
              Lege jetzt ein Vault an — entweder lokal-only (nur auf diesem Server) oder mit
              Git-Sync gegen Forgejo / GitHub / GitLab. Bestehendes Vault aus einem Remote-Repo
              clonen ist auch eine Option.
            </p>
            <Button onClick={() => void navigate({ to: '/vault/setup' })} data-testid="vault-setup-cta">
              Vault einrichten
            </Button>
          </CardContent>
        </Card>
