...
**Etappe 1 (Reskinning):** Phase A + B + **C done** (Steps 3.2 + 3.3 + 3.4, Issues [#14](https://github.com/oliverhees/lokyy/issues/14) + [#15](https://github.com/oliverhees/lokyy/issues/15) + [#16](https://github.com/oliverhees/lokyy/issues/16)). Inner workspace pinned to `v2.3.0`, branch `lokyy/main` active. Token-Layer + `@theme inline` + 13 restyled + 8 new Shadcn-canonical components + LokyyShell/AppSidebar/AppHeader parallel zu Hermes-Shell hinter `VITE_LOKYY_LAYOUT` flag. Plus `VITE_LOKYY_SKIP_SPLASH` für Visual-Verify-Bypass. Hermes-Code unangetastet — beide Flags default OFF.
