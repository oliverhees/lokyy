...
|------|--------|----------------|
| `src/styles.css` | Phase A — bridge block in each of 8 `[data-theme='claude-*']`. Phase B — `@theme inline` block. **Phase A fixup ([#25](https://github.com/oliverhees/lokyy/issues/25)) — 23 Hermes `--theme-*` aliases.** **Phase A++ ([#26](https://github.com/oliverhees/lokyy/issues/26)) — 16 additional component-level aliases for `--composer-*`, `--chat-user-*`, `--chat-assistant-*`, `--code-*`, `--tool-card-*` (each pointing at the matching Lokyy bridge var). Together: 39 aliases per theme × 8 themes.** Hermes variable NAMES stay (preserves component refs); only their VALUES now resolve through the Lokyy bridge. | Etappe 1 / [#14](https://github.com/oliverhees/lokyy/issues/14) + [#15](https://github.com/oliverhees/lokyy/issues/15) + [#25](https://github.com/oliverhees/lokyy/issues/25) + [#26](https://github.com/oliverhees/lokyy/issues/26) |
| `src/components/backend-unavailable-state.tsx` | Phase A++ ([#26](https://github.com/oliverhees/lokyy/issues/26)) — restyled as Shadcn-canonical info card: `rounded-2xl border border-border bg-card shadow-lg`, indigo-tinted circular icon container (`bg-primary/10 text-primary`), `text-2xl` title, `text-muted-foreground` body, collapsible "How to enable" details with `hermes gateway run` snippet. "Hermes Agent gateway" copy → "Lokyy Agent gateway" for visible UX strings. Used by Jobs, Skills, MCP routes — one component, three modern pages. | Etappe 1 / [#26](https://github.com/oliverhees/lokyy/issues/26) |
| `src/screens/memory/memory-browser-screen.tsx` | Phase A++ ([#26](https://github.com/oliverhees/lokyy/issues/26)) — left/right panel surfaces now use `border border-border bg-card`, the previous flat `bg-neutral-800`/`primary-200` references swapped to Lokyy Shadcn tokens. New `EmptyMemoryState` component (Brain icon in indigo-tinted circle, headline "No memory files yet", explanatory body, dashed-CTA "Drop a .md file into memory/"). File-row and code-viewer surfaces follow the same token swap. | Etappe 1 / [#26](https://github.com/oliverhees/lokyy/issues/26) |
| `src/components/agent-view/agent-view-panel.tsx` | Phase A+++ ([#27](https://github.com/oliverhees/lokyy/issues/27)) — 16 hardcoded Tailwind palette utilities replaced with Lokyy tokens: `bg-emerald-{400,500}` → `bg-primary`, `text-emerald-*` → `text-primary`, `border-emerald-{300,400}/* ` → `border-primary/30`, `bg-amber-*` / `text-amber-*` / `bg-yellow-*` → `text-muted-foreground` / `bg-muted`. `OrchestratorAvatar` (Hermes anime-character, 1700 LOC custom-painted SVG) **reference** replaced inside Inspector header with a Sally-style initial-in-indigo-circle pattern (`rounded-full bg-primary/15` + `text-primary font-semibold`). The `OrchestratorAvatar` component file itself stays untouched per CLAUDE.md branding-rule (code identifier filename + huge surface area). | Etappe 1 / [#27](https://github.com/oliverhees/lokyy/issues/27) |
| `docs/swarm-redesign-proposal.md` | New OUTER doc by Sally ([#28](https://github.com/oliverhees/lokyy/issues/28)) — 3 design options for Swarm redesign with ASCII mockups, pros/cons, cost estimates, Sally's recommendation (Option A — Hero Mission + Compact Roster), plus a decision-block for Oliver to pick. **No implementation in Etappe 1.** Implementation happens in Etappe 2 once Oliver picks. Issue #28 stays open until then. | Etappe 1 / [#28](https://github.com/oliverhees/lokyy/issues/28) |
